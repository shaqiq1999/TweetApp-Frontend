...
export class TweetsDataService {
  ...
  getAllTweets() {
    ...
    return this.http.get<Tweets[]>(baseUrl+"/api/v1/tweets/all");

  }

  getTweet(uniqueId: number) {
    return this.http.get<Tweets>(baseUrl+`/api/v1/tweet/${uniqueId}`)
  }

  postTweets(userName: String, tweets: Tweets) {
    return this.http.post(baseUrl+`/api/v1/tweets/${userName}/add`, tweets,{
  ...
  getTweetsofUser(userName: string) {
    ...
    return this.http.get<Tweets[]>(baseUrl+`/api/v1/tweets/${userName}`)
  }

  deleteTweet(userName:string,uniqueId: number) {
    return this.http.delete(baseUrl+`/api/v1/tweets/${userName}/delete/${uniqueId}`,{
      observe:'body',responseType:'text'as 'json'
    })
  }

  updateTweet(userName:string,uniqueId: number, tweet: Tweets) {
    return this.http.put(baseUrl+`/api/v1/tweets/${userName}/update/${uniqueId}`, tweet,{
      observe:'body',responseType:'text'as 'json'
    })
  }
  setLikes(userName:string,id:string) :Observable<Object>{
    ...
    return this.http.put<Object>(baseUrl+`/api/v1/tweets/${userName}/like/${id}`,"",{
      observe:'body',responseType:'text'as 'json'
    });
  }
  ...
  getComments(tweetId: number) {
    return this.http.get<Comments[]>(baseUrl+`/api/v1/comments/${tweetId}`)
  }

  postComment(userName:string,id:string, tweetreply: Comments) {
    return this.http.post(baseUrl+`/api/v1/tweets/${userName}/reply/${id}`, tweetreply,{
      observe:'body',responseType:'text'as 'json'
    })
  }
